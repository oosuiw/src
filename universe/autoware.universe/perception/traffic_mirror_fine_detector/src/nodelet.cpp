...
#include "traffic_mirror_fine_detector/nodelet.hpp"
...
TrafficMirrorFineDetectorNodelet::TrafficMirrorFineDetectorNodelet(
  const rclcpp::NodeOptions & options)
: Node("traffic_mirror_fine_detector_node", options)
{
  int num_class = 2;
  ...
  if (!readLabelFile(label_path, tmr_id_, num_class)) {
  ...
  timer_ = rclcpp::create_timer(
    this, get_clock(), 100ms, std::bind(&TrafficMirrorFineDetectorNodelet::connectCb, this));

  std::lock_guard<std::mutex> lock(connect_mutex_);
  output_roi_pub_ = this->create_publisher<TrafficMirrorRoiArray>("~/output/rois", 1);
  exe_time_pub_ =
  ...
  if (is_approximate_sync_) {
    ...
    approximate_sync_->registerCallback(
      std::bind(&TrafficMirrorFineDetectorNodelet::callback, this, _1, _2, _3));
  } else {
...
//KMS_250409
void TrafficMirrorFineDetectorNodelet::connectCb()
{
  std::lock_guard<std::mutex> lock(connect_mutex_);
  if (output_roi_pub_->get_subscription_count() == 0) {
    RCLCPP_DEBUG(this->get_logger(), "UnSubscribed...");
    image_sub_.unsubscribe();
    rough_roi_sub_.unsubscribe();
    expect_roi_sub_.unsubscribe();
  } else if (!image_sub_.getSubscriber()) {
    RCLCPP_DEBUG(this->get_logger(), "Subscribed...");
...
void TrafficMirrorFineDetectorNodelet::callback(
  const sensor_msgs::msg::Image::ConstSharedPtr in_image_msg,
  const TrafficMirrorRoiArray::ConstSharedPtr rough_roi_msg,
  const TrafficMirrorRoiArray::ConstSharedPtr expect_roi_msg)
{
  ...
  TrafficMirrorRoiArray out_rois;
  std::map<int, TrafficMirrorRoi> id2expectRoi;
  std::map<int, tensorrt_yolox::ObjectArray> id2detections;
  for (const auto & expect_roi : expect_roi_msg->rois) {
    id2expectRoi[expect_roi.traffic_mirror_id] = expect_roi;
  }
  ...
  for (size_t roi_i = 0; roi_i < rough_roi_msg->rois.size(); roi_i++) {
    ...
    lts.emplace_back(lt);
    roi_ids.emplace_back(rough_roi.traffic_mirror_id);
    ...
    if (static_cast<int>(rois.size()) == batch_size_) {
      ...
      for (size_t batch_i = 0; batch_i < true_batch_size; batch_i++) {
        for (const tensorrt_yolox::Object & detection : inference_results[batch_i]) {
          if (detection.score < score_thresh_ || detection.type != tmr_id_) {
            continue;
          }
          cv::Point lt_roi(
...
float TrafficMirrorFineDetectorNodelet::evalMatchScore(
  std::map<int, TrafficMirrorRoi> & id2expectRoi,
  std::map<int, tensorrt_yolox::ObjectArray> & id2detections,
...
void TrafficMirrorFineDetectorNodelet::detectionMatch(
  std::map<int, TrafficMirrorRoi> & id2expectRoi,
  std::map<int, tensorrt_yolox::ObjectArray> & id2detections, TrafficMirrorRoiArray & out_rois)
{
  float max_score = 0.0f;
  ...
  for (const auto & roi_pair : id2expectRoi) {
    ...
    for (const tensorrt_yolox::Object & det : id2detections[tlr_id]) {
      ...
      std::map<int, TrafficMirrorRoi> id2expectRoi_copy = id2expectRoi;
  ...
  for (const auto & p : bestDetections) {
    TrafficMirrorRoi tlr;
    tlr.traffic_mirror_id = p.first;
...
bool TrafficMirrorFineDetectorNodelet::rosMsg2CvMat(
...
bool TrafficMirrorFineDetectorNodelet::fitInFrame(
...
bool TrafficMirrorFineDetectorNodelet::readLabelFile(
...
RCLCPP_COMPONENTS_REGISTER_NODE(traffic_mirror::TrafficMirrorFineDetectorNodelet)
