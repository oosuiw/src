...
: Filter("ScanGroundFilter", options)
{
  // set initial parameters
  {
    ...
    //SWS_241129
    depth_m_ = static_cast<float>(declare_parameter("depth_m", 0.3));
    angle_rad_ = static_cast<float>(declare_parameter("angle_rad", 1.0));
    //SWS_241130
    value_int_ = declare_parameter("value_int", 240); //270
    value_int2_ = declare_parameter("value_int2", 60);
    value_float_ = declare_parameter("value_float", 0.2);
    value_float2_ = declare_parameter("value_float2", 0.3);
...
{
  //RCLCPP_INFO(get_logger(), "radial_dividers_num: %ld", radial_dividers_num_); //SWS_241129
  ...
  for (size_t i = 0; i < in_cloud->points.size(); ++i) {
    ...
    //SWS_241129
    //RCLCPP_INFO(get_logger(), "grid_id: %d, radius: %f, theta: %f, radial_div: %ld", grid_id, radius, theta, radial_div);
  ...
  PointRef & p, const std::vector<GridCenter> & gnd_grids_list)
{
  ...
  //RCLCPP_INFO(get_logger(), "global_slope: %f", curr_gnd_slope_rad); //SWS_241129
  ...
  //RCLCPP_INFO(get_logger(), "local_slope: %f", local_slope); //SWS_241129
  /*
  RCLCPP_INFO(get_logger(), "global_slope: %f, local_slope: %f, orig_point->x: %f, orig_point->y: %f, orig_point->z: %f, radius: %f, radial_div: %ld, grid_id: %d", 
    curr_gnd_slope_rad, local_slope, p.orig_point->x, p.orig_point->y, p.orig_point->z, p.radius, p.radial_div, p.grid_id);
  */
  //RCLCPP_INFO(get_logger(), "orig_point->z: %f, next_gnd_z: %f, gnd_z_local_thresh: %f, local_slope: %f", p.orig_point->z, next_gnd_z, gnd_z_local_thresh, local_slope);
  //RCLCPP_INFO(get_logger(), "orig_point->z: %f, next_gnd_z: %f, height: %f, local_slope: %f", p.orig_point->z, next_gnd_z, p.orig_point->z - next_gnd_z, local_slope);
  if (
    abs(p.orig_point->z - next_gnd_z) <= non_ground_height_threshold_ + gnd_z_local_thresh || //0.2
    abs(local_slope) <= local_slope_max_angle_rad_) { //10 -> 0.174
    p.point_state = PointLabel::GROUND;
    //RCLCPP_INFO(get_logger(), "GROUND: global_slope: %f, local_slope: %f, orig_point->x: %f, orig_point->y: %f, orig_point->z: %f, radius: %f, radial_div: %ld, grid_id: %d", 
    //curr_gnd_slope_rad, local_slope, p.orig_point->x, p.orig_point->y, p.orig_point->z, p.radius, p.radial_div, p.grid_id);
    //RCLCPP_INFO(get_logger(), "GROUND");
  } 
  if (
    p.orig_point->z - next_gnd_z > non_ground_height_threshold_ + gnd_z_local_thresh) { //else if
    p.point_state = PointLabel::NON_GROUND;
    //RCLCPP_INFO(get_logger(), "NO_GROUND: global_slope: %f, local_slope: %f, orig_point->x: %f, orig_point->y: %f, orig_point->z: %f, radius: %f, radial_div: %ld, grid_id: %d", 
    //curr_gnd_slope_rad, local_slope, p.orig_point->x, p.orig_point->y, p.orig_point->z, p.radius, p.radial_div, p.grid_id);
    //RCLCPP_INFO(get_logger(), "NO_GROUND");
  }
  //if ((p.point_state != PointLabel::GROUND) && (p.point_state != PointLabel::NON_GROUND))
  //  RCLCPP_INFO(get_logger(), "OUT_OF_RANGE");
}
  ...
  PointRef & p, const std::vector<GridCenter> & gnd_grids_list)
{
  ...
  //if (local_slope < -local_slope_max_angle_rad_) RCLCPP_INFO(get_logger(), "orig_point->z: %f", p.orig_point->z); //SWS_241129
  if (
  ...
  PointRef & p, const std::vector<GridCenter> & gnd_grids_list)
{
  ...
  float tmp_delta_radius = p.radius - gnd_grids_list.back().radius;
  float local_slope = std::atan(tmp_delta_avg_z / tmp_delta_radius);
  //RCLCPP_INFO(get_logger(), "local_slope: %f", local_slope); //SWS_241131
  if (abs(local_slope) < global_slope_max_angle_rad_) {
    ...
    //RCLCPP_INFO(get_logger(), "NO_GROUND");
  //} else {
  } else if (local_slope < -value_float_ && p.orig_point->z < -value_float2_) {
    p.point_state = PointLabel::OUT_OF_RANGE;
    RCLCPP_INFO(get_logger(), "OUT_OF_RANGE");
    //RCLCPP_INFO(get_logger(), "local_slope: %f", local_slope);
    //RCLCPP_INFO(get_logger(), "local_slope: %f, orig_point->x: %f, orig_point->y: %f, orig_point->z: %f, radius: %f, radial_div: %ld, grid_id: %d", 
    //local_slope, p.orig_point->x, p.orig_point->y, p.orig_point->z, p.radius, p.radial_div, p.grid_id);
  }
}
...
void ScanGroundFilterComponent::classifyPointCloudGridScan(
  ...
  pcl::PointIndices & out_ground_indices) //SWS_241201
{
  out_no_ground_indices.indices.clear();
  out_ground_indices.indices.clear(); //SWS_241201
  //RCLCPP_INFO(get_logger(), "in_radial_ordered_clouds.size(): %ld", in_radial_ordered_clouds.size()); //360 //SWS_241130
  //for (size_t i = 0; i < in_radial_ordered_clouds.size(); ++i) {
  for (int i = value_int_; i < value_int_ + value_int2_; ++i) {
    ...
    //RCLCPP_INFO(get_logger(), "i: %d", i); //SWS_241129
    ...
    for (size_t j = 0; j < in_radial_ordered_clouds[i].size(); ++j) {
      ...
      if (
        ...
        //RCLCPP_INFO(get_logger(), "CONTINUE"); //SWS_241129
        checkContinuousGndGrid(*p, gnd_grids);
      } else if (p->radius - gnd_grids.back().radius < gnd_grid_continual_thresh_ * p->grid_size) {
        //RCLCPP_INFO(get_logger(), "DISCONTINUE");
        checkDiscontinuousGndGrid(*p, gnd_grids);
      } else {
        //RCLCPP_INFO(get_logger(), "BREAK");
        checkBreakGndGrid(*p, gnd_grids);
      }
      //SWS_241131
      if (p->point_state == PointLabel::OUT_OF_RANGE) { //GROUND //NON_GROUND
        out_no_ground_indices.indices.push_back(p->orig_index);
      } else if (p->point_state == PointLabel::NON_GROUND) { //GROUND
        ground_cluster.addPoint(p->radius, p->orig_point->z, p->orig_index);
        out_ground_indices.indices.push_back(p->orig_index); //SWS_241201
      }
      prev_p = p;
    }
  }
...
void ScanGroundFilterComponent::extractObjectPoints(  // NON_GROUND
...
//SWS_241201
void ScanGroundFilterComponent::extractGroundPoints(  // GROUND
  const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr, const pcl::PointIndices & in_indices,
  pcl::PointCloud<pcl::PointXYZ>::Ptr out_ground_cloud_ptr)
{
  for (const auto & i : in_indices.indices) {
  ...
  PointCloud2 & output)
{
  ...
  if (elevation_grid_mode_) {
    convertPointcloudGridScan(current_sensor_cloud_ptr, radial_ordered_points); //SWS_241129
    classifyPointCloudGridScan(radial_ordered_points, no_ground_indices, ground_indices); //SWS_241201
  } else {
  ...
  extractObjectPoints(current_sensor_cloud_ptr, no_ground_indices, no_ground_cloud_ptr); //NON_GROUND
  extractGroundPoints(current_sensor_cloud_ptr, ground_indices, ground_cloud_ptr); //GROUND //SWS_241201
  ...
  const std::vector<rclcpp::Parameter> & p)
{
  ...
  //SWS_241130
  if (get_param(p, "value_int", value_int_)) {
    RCLCPP_DEBUG(get_logger(), "Setting value_int to: %d.", value_int_);
  }
  if (get_param(p, "value_int2", value_int2_)) {
    RCLCPP_DEBUG(get_logger(), "Setting value_int2 to: %d.", value_int2_);
  }
  if (get_param(p, "value_float", value_float_)) {
    RCLCPP_DEBUG(get_logger(), "Setting value_float to: %f.", value_float_);
  }
  if (get_param(p, "value_float2", value_float2_)) {
    RCLCPP_DEBUG(get_logger(), "Setting value_float2 to: %f.", value_float2_);
  }
