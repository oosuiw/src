...
#include "traffic_mirror_visualization/traffic_mirror_map_visualizer/node.hpp"
...
{
TrafficMirrorMapVisualizerNode::TrafficMirrorMapVisualizerNode(
  const std::string & node_name, const rclcpp::NodeOptions & node_options)
: rclcpp::Node(node_name, node_options)
{
  mirror_marker_pub_ =
    create_publisher<visualization_msgs::msg::MarkerArray>("~/output/traffic_mirror", 1);
  tm_state_sub_ = create_subscription<autoware_perception_msgs::msg::TrafficSignalArray>(
    "~/input/tm_state", 1,
    std::bind(&TrafficMirrorMapVisualizerNode::trafficSignalsCallback, this, _1));
  vector_map_sub_ = create_subscription<autoware_auto_mapping_msgs::msg::HADMapBin>(
    "~/input/vector_map", rclcpp::QoS{1}.transient_local(),
    std::bind(&TrafficMirrorMapVisualizerNode::binMapCallback, this, _1));
}
void TrafficMirrorMapVisualizerNode::trafficSignalsCallback(
  const autoware_perception_msgs::msg::TrafficSignalArray::ConstSharedPtr input_traffic_signals)
{
...
#if 0
  for (auto tmi = aw_tm_reg_elems_.begin(); tmi != aw_tm_reg_elems_.end(); tmi++) {
    for (const auto & lsp : (*tmi)->trafficMirrors()) {
      if (lsp.isLineString()) { // traffic mirrors can either polygons or
                                // linestrings
        ...
        for (const auto & input_traffic_signal : input_traffic_signals->signals) {
          ...
          lanelet::visualization::initTrafficMirrorTriangleMarker(&marker, "traffic_mirror_triangle");
          lanelet::visualization::pushTrafficMirrorTriangleMarker(&marker, ls, color);
          marker.header.frame_id = "map";
          marker.header.stamp = current_time;
          marker.frame_locked = true;
          marker.lifetime = rclcpp::Duration::from_seconds(0.2);
  ...
  for (auto tmi = aw_tm_reg_elems_.begin(); tmi != aw_tm_reg_elems_.end(); tmi++) {
    for (auto ls : (*tmi)->lightBulbs()) {
      if (!ls.hasAttribute("traffic_mirror_id")) {
        continue;
      }
      for (const auto & input_traffic_signal : input_traffic_signals->signals) {
        if ((*tmi)->id() == input_traffic_signal.traffic_signal_id) {
          // if (isAttributeValue(ls, "traffic_mirror_id", input_traffic_signal.map_primitive_id)) {
          for (auto pt : ls) {
            ...
            for (const auto & elem : input_traffic_signal.elements) {
              ...
              if (
                ...
                lightAsMarker(
                  get_node_logging_interface(), pt, &marker, "traffic_mirror", current_time);
              } else if (  // NOLINT
  ...
  mirror_marker_pub_->publish(output_msg);
}

void TrafficMirrorMapVisualizerNode::binMapCallback(
  const autoware_auto_mapping_msgs::msg::HADMapBin::ConstSharedPtr input_map_msg)
{
  ...
  aw_tm_reg_elems_ = lanelet::utils::query::autowareTrafficMirrors(all_lanelets);
}
}  // namespace traffic_mirror
