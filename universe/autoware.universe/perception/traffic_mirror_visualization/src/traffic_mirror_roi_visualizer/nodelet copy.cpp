...
: Node("traffic_mirror_roi_visualizer_node", options)
{
  ...
  if (enable_fine_detection_) {
    ...
    sync_with_rough_roi_->registerCallback(
      std::bind(&TrafficMirrorRoiVisualizerNodelet::imageRoughRoiCallback, this, _1, _2, _3, _4));
  } else {
    sync_.reset(new Sync(SyncPolicy(10), image_sub_, roi_sub_, traffic_signals_sub_));
    sync_->registerCallback(
      std::bind(&TrafficMirrorRoiVisualizerNodelet::imageRoiCallback, this, _1, _2, _3));
  }
  ...
  const cv::Scalar & color)
{
  ...
  cv::putText(
    ...
    cv::Point(tm_roi.roi.x_offset, tm_roi.roi.y_offset), cv::FONT_HERSHEY_COMPLEX, 1.0, color, 1,
    CV_AA);
  return true;
}
  ...
  const ClassificationResult & result)
{
  ...
  cv::putText(
    ...
    cv::Point(tm_roi.roi.x_offset, tm_roi.roi.y_offset - (offset * 0)), cv::FONT_HERSHEY_COMPLEX,
    1.1, color, 3);

  cv::putText(
    image, result.label, cv::Point(tm_roi.roi.x_offset, tm_roi.roi.y_offset - (offset * 1)),
    ...
    input_traffic_signals_msg)
{
  ...
  try {
    ...
    for (auto tm_roi : input_tm_roi_msg->rois) {
...
void TrafficMirrorRoiVisualizerNodelet::imageRoughRoiCallback(
  const sensor_msgs::msg::Image::ConstSharedPtr & input_image_msg,
  const tier4_perception_msgs::msg::TrafficMirrorRoiArray::ConstSharedPtr & input_tm_roi_msg,
  const tier4_perception_msgs::msg::TrafficMirrorRoiArray::ConstSharedPtr & input_tm_rough_roi_msg,
